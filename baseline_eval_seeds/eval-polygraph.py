...
from lm_polygraph.estimators import MahalanobisDistanceSeq, RDESeq, RelativeMahalanobisDistanceSeq
...
def eval(cfg):
    ...
    with torch.no_grad():
        ...
        mh_dist_estimator = MahalanobisDistanceSeq("encoder", normalize=False)
        rde_dist_estimator = RDESeq("encoder", normalize=False)
        rmd_dist_estimator = RelativeMahalanobisDistanceSeq("encoder", normalize=False)

        md_relative = rmd_dist_estimator(
            {'embeddings_encoder': test_features.float().cpu().numpy(),
             'train_embeddings_encoder': train_features.float().cpu().numpy(),
             'background_train_embeddings_encoder': train_features.float().cpu().numpy()
             }
        )

        md = mh_dist_estimator(
            {'embeddings_encoder': test_features.float().cpu().numpy(),
             'train_embeddings_encoder': train_features.float().cpu().numpy(),
             'background_train_embeddings_encoder': train_features.float().cpu().numpy()
             }
        ...
        n_components = 256
        rde_roc_auc = {
            f'rde_n_components_{n_components}_roc_auc' : roc_auc_score(
                errors, rde_dist
            )
        }
