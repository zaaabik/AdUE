...
class HellaSwag(BaseDataset):
    ...
    def name(self) -> str:
        """Return name of dataset."""
        if self.debug:
            return f"HellaSwag_{self.n_shot}_shot_debug"
        return f"HellaSwag_{self.n_shot}_shot"
    ...
    def get_choices(self):
        if self.debug:
            data = load_dataset(f'zaaabik/hellaswag_{self.n_shot}_shot_debug')
        else:
            data = load_dataset(f'zaaabik/hellaswag_{self.n_shot}_shot')
        return data['train'][0]['choices']
...
class TruthfulQA(BaseDataset):
    ...
    def name(self) -> str:
        """Return name of dataset."""
        if self.debug:
            return f"truthfulqa_{self.n_shot}_shot_debug"
        return f"truthfulqa_{self.n_shot}_shot"
    ...
    def get_choices(self):
        ...
        length = [len(i['choices']) for i in data['train']]
        return data['train'][np.argmax(length)]['choices']
